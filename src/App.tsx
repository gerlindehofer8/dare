...
import { cloudIdentity, fetchCloudDares, requestMagicLink, signOutCloud, syncCloudDares } from './services/customDares'
...
function Settings({ value, onChange, onBack }: { value: GameSettings; onChange: (s: GameSettings) => void; onBack: () => void }) {
  ...
  const sendLink = async (event: FormEvent) => { event.preventDefault(); const error = await requestMagicLink(email.trim()); setSyncMessage(error ? 'Der Link konnte nicht gesendet werden. Prüfe E-Mail und Supabase-Einstellungen.' : 'Link gesendet. Öffne ihn auf diesem Gerät, um die Synchronisierung zu verbinden.') }
