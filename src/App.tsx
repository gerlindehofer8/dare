...
import { fetchCloudDares, syncCloudDares } from './services/customDares'
...
function App() {
  ...
  useEffect(() => { if (!supabase) return; void fetchCloudDares().then(remote => { if (!remote) return; cloudReady.current = true; setCustomDares(local => Array.from(new Map([...local, ...remote].map(dare => [dare.id, dare])).values())) }) }, [])
  ...
  if (view === 'dares') return <DareManager dares={customDares} setDares={setCustomDares} onBack={() => go('home')} />
...
function DareManager({ dares, setDares, onBack }: { dares: Dare[]; setDares: (d: Dare[]) => void; onBack: () => void }) {
